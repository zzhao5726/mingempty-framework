...
public abstract class AbstractDelegatingContext<V> implements Comparable<AbstractDelegatingContext<V>> {
    ...
    @Getter
    @JsonIgnore
    @Schema(title = "是否开启事务记录", description = "是否开启事务记录")
    private boolean enableTx = false;

    /**
     * 事务ID
     * <br>
     * 当该值不为空时，说明进入了事务模式，在执行结束后需要修改数据库内任务状态
     */
    @Getter
    @JsonIgnore
    @Schema(title = "事务ID", description = "事务ID")
    private String txId;
    ...
    protected final V abstractRun() {
        ...
        try {
            ...
            try {
                ...
                if (!isError && StrUtil.isNotEmpty(this.getTxId())) {
                    TaskUtil.complete(this.getTxId());
                }
    ...
    public void setEnableTx(boolean enableTx) {
        ...
        if (enableTx) {
            this.txId = UUID.fastUUID().toString(true);
        }
    }
