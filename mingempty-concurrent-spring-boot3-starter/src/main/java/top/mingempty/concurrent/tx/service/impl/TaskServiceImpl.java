package top.mingempty.concurrent.tx.service.impl;
...
import top.mingempty.concurrent.tx.mapper.TaskMapper;
import top.mingempty.concurrent.tx.model.TaskPo;
import top.mingempty.concurrent.tx.service.TaskService;
import top.mingempty.concurrent.tx.util.TaskUtil;
...
public class TaskServiceImpl implements TaskService {
    ...
    public <T extends AbstractDelegatingContext<V>, V> void record(T contex, String poolName) {
        TaskPo taskPo = taskMapper.selectByPrimaryKey(contex.getTxId());
        if (taskPo != null) {
            return;
        }
        taskPo = new TaskPo();
        taskPo.setTaskId(contex.getTxId());
    ...
    public void retry(String taskId) {
        ...
        if (object instanceof AbstractDelegatingContext abstractDelegatingContext) {
            abstractDelegatingContext.setTxId(taskPo.getTaskId());
        }
