...
(同字典条目编码：dict_entry_type)',
	entry_status char default '1' not null comment '条目启用状态
0：未启用
1：启用
(同字典条目编码：enable_or_not)',
	entry_category varchar(150) not null comment '条目类别
...
(同字典条目表条目编号)',
	...
	item_status char default '1' not null comment '字典启用状态
0：未启用
1：启用
(同字典条目编码：enable_or_not)',
...
(
	label_id bigint unsigned not null comment '字典ID'
		primary key,
	entry_code varchar(64) not null comment '条目编号
(同字典条目表条目编号)',
	entry_version int unsigned default 1 not null comment '条目版本（默认1）',
...
(含义同条目：dict_label)',
	item_code varchar(64) not null comment '字典项编号',
	label_status char default '1' not null comment '标签启用状态
0：未启用
1：启用
(同字典条目编码：enable_or_not)',
	create_time datetime default CURRENT_TIMESTAMP not null comment '创建时间',
...
(
	...
	entry_code varchar(64) not null comment '条目编号
(同字典条目表条目编号)',
	item_label_code varchar(64) not null comment '标签编号
(含义同条目：dict_label)',
	item_code varchar(64) not null comment '字典项编号',
	create_time datetime default CURRENT_TIMESTAMP not null comment '创建时间',
	update_time datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
	...
	constraint ui_l_ec_lc_ic
		unique (entry_code, item_label_code, item_code)
)
comment '字典项标签表';
