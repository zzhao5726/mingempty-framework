...
public interface EntryMapstruct {
    ...
    @Named("toChangePo")
    ChangeEntryPo toChangePo(EntryCreateVo entryCreateVo);

    @IterableMapping(qualifiedByName = "toChangePo")
    List<ChangeEntryPo> toChangePo(Collection<EntryCreateVo> entryCreateVos);
    ...
    EntryResultVo queryBoToResultVo(EntryBo entryBo);

    @IterableMapping(qualifiedByName = "queryBoToResultVo")
    List<EntryResultVo> queryBoToResultVo(List<EntryBo> entryBos);
}
