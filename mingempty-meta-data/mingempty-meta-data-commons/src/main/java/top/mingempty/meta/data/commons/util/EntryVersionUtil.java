...
public class EntryVersionUtil {
    ...
    public static void settingVersion(Map<String, Long> entryVersionMap) {
        ...
        synchronized (Thread.currentThread()) {
            Map<String, Long> entryVersionMapByCurrent = ENTRY_VERSION_THREAD_LOCAL.get();
            entryVersionMap.putAll(entryVersionMapByCurrent);
            ENTRY_VERSION_THREAD_LOCAL.set(entryVersionMap);
        }
    }
