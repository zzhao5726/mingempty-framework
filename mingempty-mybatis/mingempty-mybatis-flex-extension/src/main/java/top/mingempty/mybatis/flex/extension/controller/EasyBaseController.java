...
package top.mingempty.mybatis.flex.extension.controller;
...
import top.mingempty.mybatis.flex.extension.entity.MfPage;
import top.mingempty.mybatis.flex.tool.MfWrapperTool;
 ...
 * @author zzhao
 */
public abstract class EasyBaseController<S extends IService<T>, T> {
    ...
    public MeRsp<T> list(@RequestBody MeReq<MePubConditions> meReq) {
        QueryWrapper queryWrapper = MfWrapperTool.entityToWrapper(meReq.getData());
        MePage mePage = meReq.getMePage();
        if (ObjUtil.isNotEmpty(mePage)) {
            Page<T> page = baseService.page(new MfPage<>(mePage), queryWrapper);
            return MeRsp.success(page.getRecords(), mePage);
        }
    ...
    public MeRsp<T> updateBatch(@RequestBody MeReq<T> meReq) {
        if (baseService.updateBatch(meReq.getDataList())) {
            return MeRsp.success();
        }
